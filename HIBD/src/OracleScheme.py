...
from src.Util import generateRandomString, generateRandomDate
...
class OracleScheme:
    ...
    def generate_data(self, start_person_id, other_id):
        ...
        subdivisions = []
        for i in range(5):
            subdivisions.append(self.Subdivision_oracle(name=generateRandomString(20),
                                                        type="f"))
        ...
        subjects = []
        for i in range(5):
            subjects.append(self.Subject_oracle(name=generateRandomString(10)))

        program = []
        for i in range(5):
            program.append(self.Program_oracle(id=self.id, name=generateRandomString(10), code=generateRandomString(6)))
            self.id = self.id + 1

        specializations = []
        for i in range(10):
            specializations.append(
                self.Specialization_oracle(id=self.id, name=generateRandomString(10), code=generateRandomString(6),
        ...
        for i in range(20):
            groups.append(
                self.Group_oracle(id=self.id, name=generateRandomString(4), year_name=random.choice(years).name,
        ...
        for i in range(100):
            persons.append(
                self.Person_oracle(id=start_person_id, surname=generateRandomString(10), name=generateRandomString(10),
                                   patronymic=generateRandomString(10),
                                   dateofbirth=generateRandomDate(),
                                   placeofbirth=generateRandomString(20)))
        ...
        for i in range(20):
            employees.append(
                self.Employee_oracle(id=self.id, person_id=random.choice(persons).id,
                                     subdivision_name=random.choice(subdivisions).name,
                                     position=generateRandomString(8),
